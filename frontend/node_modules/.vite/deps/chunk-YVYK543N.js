...
function useControllableReducer(internalReducer, externalReducer, props) {
  ...
  const combinedReducer = React36.useCallback((state, action) => {
    if (externalReducer) {
  ...
  const [nextState, dispatch] = React36.useReducer(combinedReducer, initalState);
  const previousState = React36.useRef(initalState);
  React36.useEffect(() => {
...
function useMenu(parameters = {}) {
  ...
  const [menuItems, setMenuItems] = React39.useState({});
  const listboxRef = React39.useRef(null);
  const handleRef = useForkRef(listboxRef, listboxRefProp);
  const registerItem = React39.useCallback((id, metadata) => {
  ...
  const unregisterItem = React39.useCallback((id) => {
    setMenuItems((previousState) => {
      const newState = _extends({}, previousState);
      delete newState[id];
      return newState;
  ...
  const highlightFirstItem = React39.useCallback(() => {
    if (Object.keys(menuItems).length > 0) {
      setListboxHighlight(menuItems[Object.keys(menuItems)[0]].id);
    }
  }, [menuItems, setListboxHighlight]);
  const highlightLastItem = React39.useCallback(() => {
    if (Object.keys(menuItems).length > 0) {
...
var Portal = React40.forwardRef(function Portal2(props, ref) {
  ...
  const handleRef = useForkRef(React40.isValidElement(children) ? children.ref : null, ref);
  useEnhancedEffect_default(() => {
    if (!disablePortal) {
...
var ModalUnstyled = React45.forwardRef(function ModalUnstyled2(props, ref) {
 ...
 * LICENSE file in the root directory of this source tree.
 */
//# sourceMappingURL=chunk-YVYK543N.js.map
